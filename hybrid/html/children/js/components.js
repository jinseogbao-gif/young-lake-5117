Vue.component('page-footer', {
  ...
  methods: {
    secondsFormat(s) {
      ...
      return day > 0
        ? day + this.transwords[this.lang].tian
        : hour > 0
        ? hour + this.transwords[this.lang].shi
        : minute > 0
        ? minute + this.transwords[this.lang].fenz
        : second + this.transwords[this.lang].miao
    },
    hideBtn() {},
    getOpenTime() {
      initDataToken(
        {
          url: 'v2/microtrade/seconds'
        },
        (res) => {
          if (res.length > 0) {
    ...
    getCurrency() {
      ...
      initDataToken(
        {
          url: 'v2/microtrade/payable_currencies'
        },
        (res) => {
          if (res.length > 0) {
            console.log(res)
    ...
    determine() {
      ...
      initDataToken(
        {
          url: 'v2/microtrade/submit',
          type: 'post',
          data: {
            type: this.type == 'buy' ? 1 : 2,
            currency_id: this.currencyItem.id,
            forex_id: obj.currency_match_id,
            number: this.num,
                          ...
                          <div v-show="isSubmit" class="sub-select">
                             <!-- <div class="icon-text">
                              ...
                              <div class="footer-btn slide-box" >
                                  <div  class="btn-item slide-item dark:btn-item" :class="item.code==currencyItem.code?'active':''" v-for="(item,index) in currencyList" :key="index">
                                          <span>{{item.code}}</span>
                  ...
                  <div>
                      ...
                      <span>{{currencyItem.code}}</span>
                              ...
                              <!--<div class="percentage"> 
                                      <div class="text">
                                              <img src="children/image/logo-a.png" alt="">
...
Vue.component('page-list', {
  ...
  data() {
    return {
      ...
      show: false,
      set: null,
      websockUrl: `wss://${_DOMAIN.replace(/^https?\:\/\//i, '')}/api/v2/ws`,
      socket: '',
      heartbeat: null,
      currentIndex: 0,
      currencyList: [],
  ...
  methods: {
    ...
    setScroll() {
      //距离底部50px触发上拉事件
      window.onscroll = () => {
        if (this.getScrollTop() + this.getClientHeight() === this.getScrollHeight() && this.isScroll) {
          console.log('上拉加载了')
          // 发起ajax请求
          this.pageSize.page++
        ...
        (res) => {
          ...
          this.set = setInterval(() => {
            for (let i = 0; i < res.data.length; i++) {
              if (res.data[i].remain_milli_seconds <= -1) {
                res.data[i].remain_milli_seconds = -1
              } else {
            ...
            if (val != 3) {
              this.list = this.list.filter((item) => {
                if (item.remain_milli_seconds == -1) {
                ...
                return item.remain_milli_seconds > -1
              })
            }
          }, 1000)
